...
jest.mock('net', () => {
    return {
        Socket: jest.fn().mockImplementation(() => {
            ...
            emitter.injectCommand = ((cmd: Commands.ISetCommand, cmdType: ReqType = ReqType.Push, protoversion: ProtocolVersion = ProtocolVersion.V1) => {
                const json = JSON.stringify({id: (cmd.constructor as any).rawName, type: cmdType, value: cmd.serialize(protoversion) })
...
describe('gostream', () => {
    ...
    test('streamOutput PUS messages outside limits', async() => {
        ...
        expect(spy).toHaveBeenCalledWith('Failed to deserialize command: DeserializationError: streamOutput received with value out of range: 4')  
    })
    ...
    test('streamOutput has been requested three times with correct index when a connection is established', async() => {
        ...
        socket.write.mock.calls.forEach((call) => {
            ...
            if(packet.id === 'streamOutput')
                streamOutputPackages.push(packet)
        })
