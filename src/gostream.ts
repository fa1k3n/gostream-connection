...
export class BasicGoStream extends EventEmitter<GoStreamEvents> {
  ...
  private handleGoStreamPacket(msg_data: Buffer): IGetCommand[] {
    ...
    if (index !== 0) {
      if (this.partialPacketBuffer.length > 0) {
        ...
        const packet_data = Buffer.alloc(
          this.partialPacketBuffer.length + offset,
    ...
    while (index >= 0) {
      ...
      const packet_data = msg_data.subarray(
        index,
        index + PACKET_HEADER_SIZE + packet_size,
      );
      if (index + PACKET_HEADER_SIZE + packet_size > msg_data.length) {
        // Packet is not complete, save this in partial packet buffer
        this.partialPacketBuffer = Buffer.alloc(
          msg_data.length - index,
          msg_data.subarray(index),
        );
        break;
      }
      const unpackedData = this.unpackData(packet_data);
      if (unpackedData) commands.push(unpackedData);
      index = msg_data.indexOf(
        PACKET_HEAD,
        index + PACKET_HEADER_SIZE + packet_size,
      );
    }
  ...
  private unpackData(msg_data: Buffer): IGetCommand {
    try {
      ...
      if (
        cmdConstructor &&
        "deserialize" in cmdConstructor &&
        typeof cmdConstructor.deserialize === "function"
      ) {
        try {
  ...
  protected sendCommand(
    ...
    cmdType: ReqType = ReqType.Set,
  ): boolean {
    if (this._socket !== null) {
      const json = JSON.stringify({
        id: (cmd.constructor as any).rawName,
        type: cmdType,
        value: cmd.serialize(ProtocolVersion.V1),
      });
...
export class GoStream extends BasicGoStream {
  ...
  public streamInfo(
    streamIndex: number,
    set?: { enable?: boolean; platform?: string; key?: string },
  ): boolean | Streaming.StreamInfo {
    const commands: Commands.ISetCommand[] = [];

    if (set === undefined) return this._state.streaming.streamInfo[streamIndex];
    ...
    if ("platform" in set) {
      commands.push(
        new Commands.StreamPlatformCommand(streamIndex, set.platform),
      );
    }
    //  if("key" in options) {
