...
export class StreamOutputCommand extends GoStreamCommand<{enable: boolean}> {
    ...
    public static deserialize(value: (string | number)[]): StreamOutputCommand {
        ...
        if (streamIndex < 0 || streamIndex > 3)
            throw new DeserializationError("streamOutput received with value out of range: " + value[0])
    ...
    public applyToState(state: GoStreamState): string | string[] {
        state.streaming.streamInfo[this.streamIndex].enable = this.properties.enable
        return `streaming.streamInfo.` + this.streamIndex + '.enable'
    }
}

export class StreamPlatformCommand extends GoStreamCommand<{platform: string}> {
    ...
    public applyToState(state: GoStreamState): string | string[] {
        state.streaming.streamInfo[this.streamIndex].platform = this.properties.platform
...
export class LiveInfoCommand extends GoStreamCommand<{status: LiveStatus}> {
    ...
    public applyToState(state: GoStreamState): string | string[] {
        state.streaming.streamInfo[this.streamIndex].status = this.properties.status
